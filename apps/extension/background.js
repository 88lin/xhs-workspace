...
class StorageManager {
  ...
  normalizePostsForAtelier(posts) {
    return posts.map(post => ({
      ...post,
      platform: post.platform || 'xiaohongshu',
      sourceUrl: post.sourceUrl || (post.noteId ? `https://www.xiaohongshu.com/explore/${post.noteId}` : ''),
      exportSource: 'xhs-collector-extension',
    }));
  }

  async exportAtelierPackage() {
    const posts = this.normalizePostsForAtelier(await this.getAllPosts());
    const manifest = chrome.runtime.getManifest();
    return JSON.stringify({
      app: 'XHS Atelier',
      schema: 'atelier-capture-bundle@1',
      source: 'xhs-atelier-collector',
      sourceType: 'browser-extension-export',
...
async function handleExport(format) {
  ...
  switch (format) {
    case 'atelier': content = await storage.exportAtelierPackage(); filename = `xhs_atelier_import_${ds()}.json`; mimeType = 'application/json'; break;
  ...
  return {
    ...
    downloadId,
    format,
    guidance: format === 'atelier'
      ? '下载后可直接拖入 XHS Atelier 桌面端，或放入桥接目录后导入。'
      : null,
  };
}
